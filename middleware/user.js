...
//             // let cartCount;
//             //res.render('user/Index', { title: 'Home', user, data,cartCount,dataNormal });
//         }
//     } catch (err) {
//         console.log(err)
//     }
...
const userIsBlocked = async (req, res, next) => {
    try {
        if (req.session.user) {
            ...
            const user = req.session.user;
            const userDtls = await userModel.findOne({ email: email });
            console.log("userDtlsstatus>>",userDtls)
            if (userDtls.status === false) {
                next()
            } else {
                req.session.email = null;
                req.session.user = null;
